 ...
 *
 * Dwa polecenia, obie strony sprawdzenia:
 *
...
import { cycleGoals, cycles, exercises, tags, users, workoutSets } from '../schema.js';
...
export async function runSample(): Promise<void> {
  ...
  try {
    ...
    process.stderr.write(
      `Wypełniono bazę danymi próby: ${String(accounts.length)} konta, 3 serie, 1 cykl ` +
...
async function main(): Promise<void> {
  ...
  switch (command) {
    ...
    case 'compare':
      ...
      return;
    default:
      throw new Error('Użycie: drill <sample|compare>');
  }
}
